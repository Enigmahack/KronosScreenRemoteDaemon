...
def _make_cleaner_posttar(debug: bool = False) -> str:
    ...
    lines = [
        ...
        "# screenremote unloads midi_bridge + vkbd + nks4_inject ITSELF via",
        "# delete_module(2) on SIGTERM (restores OA's patched .text, frees",
        "# trampolines) -- covers every exit path, including an early SIGTERM",
        "# during boot, since 2026-07-16 (screenremote.c's graceful_shutdown()).",
        "# rmmod is NOT actually unsafe here (corrected 2026-07-16, was a",
        "# misunderstanding, confirmed by live rmmod testing with no oops): the",
        "# real hazard is narrower -- reading /proc/modules OOPSes only if a",
        "# module is still COMING (mid-init), not once it's LIVE, which is the",
        "# normal state by the time this script runs (see screenremote.c's",
        "# wait_for_oa_live() comment for the confirmed crash signature). This",
        "# script still prefers the daemon's own delete_module(2) for ordering/",
        "# precision; any module still resident (e.g. kronos_extract) is cleared",
        "# by the reboot that completes the clean.",
