...
EVA only ever tracks one keyboard fd (`CHIDDriver::KeyboardIsConnected()`, decompiled from Eva 3.2.2): it scans `/sys/class/input/event0..event3` for the first device whose `bustype` reads `"0003"` (BUS_USB) and locks onto it until that device disappears, never rescanning otherwise. If a real USB keyboard is already attached at boot, it enumerates before `vkbd.ko` loads and claims a lower event slot, so EVA binds to it instead of vkbd - vkbd's own injected keys would then land in a device EVA isn't reading. To handle this, `vkbd.ko` registers a standard kernel `input_handler` that tracks whichever external BUS_USB input device currently exists (mirroring EVA's own bustype check, including its quirk of not requiring `EV_KEY`) and injects into *that* device instead of always injecting into its own when one is present, falling back to itself when none is. This self-heals across plug/unplug/replug with no module reload and without ever displacing a real keyboard, which keeps working normally throughout.
