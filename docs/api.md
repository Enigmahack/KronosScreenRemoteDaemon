...
**Address resolution.** `RT_chord_trigger` *is* kallsyms-visible, as the mangled `_Z16RT_chord_triggerhhhh` (FUNC GLOBAL in `OA.ko`'s `.text`) - corrected 2026-08-09; this section previously claimed the opposite. screenremote probes that symbol directly, and the full mangled name is required rather than an `RT_chord_trigger` substring, which would also match `_Z20RT_chord_trigger_ntehhc`, `_Z20RT_chord_trigger_velhhh` and `_Z21RT_chord_trigger_chanhh`.

A fixed file-offset delta against `Do_KM_note_out_chord_trig` (`RT_CHORD_TRIGGER_DELTA`, `0x4bbb8`) is retained only as a cross-check - both functions live in the same `OA.ko`, so the delta survives relocation, and a disagreement between the two is logged loudly. The delta is not safe as a primary source: its only guard is against underflow, so any future OS build that moves the two functions relative to each other would yield a wrong but plausible kernel address.

If the direct symbol is absent *and* the delta anchor is gone, `PADCHORD` fails gracefully with `ERR NKS4_NOT_LOADED`-style unavailability (screenremote logs `fn_chord not resolved` at startup) while every other command keeps working - re-derive with `nm` against the current `OA.ko` if that happens (see `resolve_nks4_kallsyms()`'s own comment in `screenremote.c`).
...
**Why `DAMPER` ramps internally.** The jack accepts either a simple on/off footswitch (Korg PS-1) or a continuous half-damper pedal (Korg DS-1H), and `AnalogDamperHandler` evidently uses rate-of-change to tell them apart - this is *not* a polarity-setting effect (a polarity change was tried first, based on the odd initial symptoms, and made no difference). A direct jump to a target value produced inconsistent, non-repeatable results on real hardware across two otherwise-identical test runs; a gradual ramp through every intermediate value did not - confirmed via a 256-step sweep (1 unit per step, ~40ms/step, full `0`-`127`-`0` sweep), smooth and repeatable in both directions. `DAMPER` therefore steps to its target internally rather than jumping - see [Section 7.0](#70-front-panel-injection-architecture-as-of-1100)'s `nks4_analog_ramp_start()` reference and `screenremote.c`'s own comment for the implementation.

> **Changed in 2.0.11 - `OK` now means "accepted", not "finished".** The ramp used to run inline, blocking the daemon's entire single-threaded main loop for its whole duration (measured on hardware: `DAMPER 0`->`127` took 3.190 s, during which a concurrent `STATE` query was stalled 2.940 s - no frames, no ctrl, no MIDI drain, no stuck-note watchdog). It is now stepped from the main loop instead, so the reply returns immediately and the ramp completes in the background at the same ~25 ms/step pace. A client that needs to know the ramp has landed must poll rather than rely on the reply's timing. Sending a new `DAMPER`/`TEMPO` value while a ramp is in flight retargets it smoothly from wherever it has reached; it does not restart or queue.
...
Same `ANALOG <device_code> <value*2> 0` encoding as every other analog command, with `device_code = 26`, but sent as a ramp (one `ANALOG` call per intermediate value, ~25ms apart) from the last value this daemon commanded rather than a single jump - see `nks4_analog_ramp_start()`/`nks4_analog_ramp_pump()` in `screenremote.c`. As of 2.0.11 the ramp is stepped from the main loop rather than run inline, so `TEMPO` replies `OK` immediately (see the `DAMPER` note above). The first `TEMPO` call after daemon startup has no known starting point to ramp from and jumps directly as a best-effort default; every call after that ramps from the previous `TEMPO` call's target. A physical hand on the front-panel tempo control between `TEMPO` calls would desync this tracking, the same caveat any absolute-position software control over a real analog input has.
...
```
Request:  MODE_DETAIL\n
Response: SOURCE=<eva|pixel|none> MODE=<n> EDITCTX=<e> EDITSLOT=<s> EVA_LOADED=<0|1> EVA_RESOLVED=<0|1> EVA_STAGE=<stage|-> SM_POMMI_AUTO=<status> BOOT=<0|1>\n
```
...
| `EVA_STAGE` | Only meaningful while `EVA_RESOLVED=0` (otherwise `-`) - which hop into Eva's memory failed: `find_task` (no process named `Eva` exists right now - note this is the SAME condition `update_boot_state()`'s independent uptime-override fallback needs to see Eva to ever force `BOOT` open, so this staying `find_task` for minutes past boot means `BOOT` will not self-clear either), `read_sm_pommi`/`read_modemgr_ptr` (Eva is running, but the calibrated pointer-chain address doesn't hold what's expected for this Eva build - needs recalibration, not just more waiting) |
| `SM_POMMI_AUTO` | A separate, independent process from the three above - this daemon's own attempt to autodetect `sm_pommi_addr` for `eva_mode.ko` from Eva's on-disk ELF `.symtab` and push it live via sysfs, with a readback to confirm the kernel actually took it (see `resolve_eva_sm_pommi_addr()`/`write_eva_sm_pommi_addr()`, `source/screenremote.c`). `pending` before the first attempt; a bare reason (still retrying, NOT terminal) is `elf32_find_symbol()`'s own failure string, now with concrete numbers where one exists - `not_mounted_yet`, `open_failed_errno_<n>`, `too_small_size_<n>`, `bad_elf_magic_or_class_...`, `no_symtab_section_of_<n>`, `symbol_not_found_of_<n>`, etc.; `write_failed:<reason>` (resolved fine, but the sysfs write or its readback verification didn't stick - also still retrying, NOT terminal, e.g. `wr_open_errno_<n>` if `/sys` wasn't mounted yet at that moment); `gave_up:<last-reason>` once the retry deadline elapses with no successful+verified write, `<last-reason>` being whatever the field held the instant before giving up (not just a bare `gave_up` - an earlier version of this field threw that information away). The deadline clock only starts counting once a probe first gets past "the file doesn't exist yet" (not from boot) - a slow-to-mount `Eva.img` doesn't eat into the window this has to actually resolve/write once the file exists. Useful specifically when `EVA_STAGE` stays `read_sm_pommi` and this field explains why the correction never applied. |
...
3. **EVA UI rendering (added 2026-07-25).** Reuses, verbatim, the same framebuffer nonblack%/distinct-palette-index signal (and its debounce) already trusted elsewhere in this daemon to time `midi_bridge`'s deferred load - the boot/loading screen is a dead-flat 0%/2-index frame for its entire ~67s duration, even the darkest real UI screen (Set List) is 21%/17, "a single-second snap with an enormous margin." Entirely independent of `eva_mode.ko` and of (1)/(2) - added after a real console-less unit was observed with `EVA_STAGE=read_sm_pommi` (Eva genuinely running, `sm_pommi_addr` wrong for that build) for minutes on end while this exact signal was already firing correctly (that same boot's `midi_bridge` load succeeded on schedule). As fast as (1) normally is, and no less trustworthy than what this daemon already lets gate a live kernel-level tap into Eva's own MIDI queues - a strictly more delicate operation than accepting a `BUTTON`/`TOUCH` command. Confirms Eva is past its loading screen, not that `MODE`/`EDITCTX` are readable - `EVA_STAGE` on `MODE_DETAIL` still separately reports whether (1)'s pointer chain works.

Any of the three signals is sufficient. All are heuristics, not a true "construction complete" signal from Eva itself (no such signal exists anywhere in the driver/OS stack - see `docs/EVA_ModeManager_probe.md` and the OmapNKS4 driver research it links) - but layering a fast content-based check, a slow content-independent wall-clock backstop, and a second, differently-sourced content-based check keeps the window where all three could be fooled at once very small.
